...
func NewRouter(deps app.Dependencies) *mux.Router {
	...
	peerlySubrouter.Handle("/report_appreciations", middleware.JwtAuthMiddleware(listReportedAppreciations(deps.ReportAppreciationService), []string{constants.AdminRole})).Methods(http.MethodGet).Headers(versionHeader, v1)
