...
func NewRouter(deps app.Dependencies) *mux.Router {
	...
	router.Handle("/intranet/users", listIntranetUsersHandler(deps.UserService)).Methods(http.MethodGet)

	router.Handle("/users", middleware.JwtAuthMiddleware(listUsersHandler(deps.UserService), []string{constants.UserRole})).Methods(http.MethodGet).Headers(versionHeader, v1)
