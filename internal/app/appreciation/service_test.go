	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(apprMock *mocks.AppreciationStorer, coreValueRepo *mocks.CoreValueStorer) {
				...
				apprMock.On("HandleTransaction", mock.Anything, tx, nil).Return(nil).Once()
			},
		...
		},
		{
			...
			setup: func(apprMock *mocks.AppreciationStorer, coreValueRepo *mocks.CoreValueStorer) {
				...
				apprMock.On("HandleTransaction", mock.Anything, tx, apperrors.InvalidCoreValueData).Return(nil).Once()
			},
		...
		},
		{
			...
			setup: func(apprMock *mocks.AppreciationStorer, coreValueRepo *mocks.CoreValueStorer) {
				...
				apprMock.On("BeginTx", mock.Anything).Return(tx, nil).Once()
				coreValueRepo.On("GetCoreValue", mock.Anything, int64(1)).Return(dto.GetCoreValueResp{}, nil).Once()
				apprMock.On("IsUserPresent", mock.Anything, tx, int64(2)).Return(false, apperrors.UserNotFound).Once()
				apprMock.On("HandleTransaction", mock.Anything, tx, apperrors.UserNotFound).Return(apperrors.UserNotFound).Once()
			},
			isErrorExpected: true,
			expectedResult:  dto.Appreciation{},
