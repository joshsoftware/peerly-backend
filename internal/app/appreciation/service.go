...
func (apprSvc *service) CreateAppreciation(ctx context.Context, appreciation dto.Appreciation) (dto.Appreciation, error) {
	...
	if !chk {
		logger.Errorf(ctx, "service: User not found (user_id): %v", appreciation.Receiver)
	...
	if err != nil {
		logger.Info(ctx, "service: error in getting create appreciation sender info")
	}
...
func (apprSvc *service) ListAppreciations(ctx context.Context, filter dto.AppreciationFilter) (dto.ListAppreciationsResponse, error) {
	...
	logger.Debug(ctx, " service: ", responses, " paginationResp: ", paginationResp)
...
func (apprSvc *service) UpdateAppreciation(ctx context.Context, orgTimezone string) (bool, error) {
	...
	if err != nil {
		logger.Errorf(ctx, "service: error in begin transaction: %v", err)
		return false, err
	}
	...
	defer func() {
		...
		if txErr != nil {
			err = txErr
			logger.Infof(ctx, "service: error in handle transaction, err: %s", txErr.Error())
			return
		}
	}()
	...
	if err != nil {
		logger.Error(ctx, "service: err: ", err.Error())
		return false, err
	}
...
func (apprSvc *service) sendAppreciationNotificationToReceiver(ctx context.Context, appr repository.AppreciationResponse) {
	...
	if err != nil {
		logger.Errorf(ctx, "service: err in getting device tokens: %v", err)
		return
	}

	logger.Debug(ctx, "service: notificationTokens: ", notificationTokens)
	...
	logger.Infof(ctx, "service: message: %v", msg)
...
func (apprSvc *service) sendAppreciationNotificationToAll(ctx context.Context, appr repository.AppreciationResponse) {
	...
	logger.Debug(ctx, " service: appr: ", appr)
	msg := notification.Message{
		Title: "Appreciation",
