...
func (cron *MonthlyJob) Task(ctx context.Context) {
	logger.CronInfo(ctx, "in monthly job task")
	var err error
	for i := 0; i < 3; i++ {
		...
		logger.CronInfo(ctx, fmt.Sprintf("cronjob fail error: %v", err.Error()))
	}
	// return err
	return
}
...
func (cron *MonthlyJob) setMonthlyInterval() error {
	...
	logger.CronInfo(context.Background(), fmt.Sprintf("MONTHLY_CRON_JOB_INTERVAL_MONTHS = %d", MONTHLY_CRON_JOB_INTERVAL_MONTHS))
	return nil
}
