...
func (cron *CronJob) Execute(task func(context.Context)) {
	...
	startTime := time.Now()
	logger.CronInfof(ctx, "cron job Started at %s", startTime.Format("2006-01-02 15:04:05"))
	defer func() {
		logger.CronInfof(ctx, "cron job done %s, took: %v", cron.name, time.Since(startTime))
	}()
